...
val MIGRATION_7_9 = object : Migration(7, 9) {
    override fun migrate(connection: SQLiteConnection) {
        ...
        connection.execSQL("CREATE TABLE IF NOT EXISTS `cxf_imports` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `account_id` INTEGER NOT NULL, `cxf_item_id` BLOB NOT NULL, `type` TEXT NOT NULL, `name` BLOB NOT NULL, `url` BLOB NOT NULL, `username` BLOB NOT NULL, `password` BLOB NOT NULL, `note` BLOB NOT NULL, `created_at` INTEGER, `modified_at` INTEGER, `imported_at` INTEGER NOT NULL, `flagged_ignored` INTEGER NOT NULL, `hash` TEXT NOT NULL, FOREIGN KEY(`account_id`) REFERENCES `cxf_accounts`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE )")
        connection.execSQL("CREATE INDEX IF NOT EXISTS `index_cxf_imports_account_id` ON `cxf_imports` (`account_id`)")
        connection.execSQL("CREATE TABLE IF NOT EXISTS `cxf_passkeys` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `account_id` INTEGER NOT NULL, `cxf_item_id` BLOB NOT NULL, `rp_id` TEXT NOT NULL, `name` BLOB NOT NULL, `url` BLOB NOT NULL, `username` BLOB NOT NULL, `credential_id` BLOB NOT NULL, `user_handle` BLOB NOT NULL, `note` BLOB NOT NULL, `created_at` INTEGER, `modified_at` INTEGER, `imported_at` INTEGER NOT NULL, `flagged_ignored` INTEGER NOT NULL, `hash` TEXT NOT NULL, FOREIGN KEY(`account_id`) REFERENCES `cxf_accounts`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE )")
