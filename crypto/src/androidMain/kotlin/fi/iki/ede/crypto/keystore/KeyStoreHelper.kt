...
import fi.iki.ede.crypto.keystore.CipherUtilities.Companion.AES_CBC_PKCS5_PADDING
import fi.iki.ede.crypto.keystore.CipherUtilities.Companion.AES_CBC_PKCS7_PADDING
...
import fi.iki.ede.crypto.keystore.CipherUtilities.Companion.SALT_SIZE
...
//    }
    ...
    private fun getAESCipher(): Cipher {
        return try {
            Cipher.getInstance(AES_CBC_PKCS7_PADDING)
        } catch (e: NoSuchAlgorithmException) {
            Cipher.getInstance(AES_CBC_PKCS5_PADDING)
    ...
    internal fun encryptByteArray(
        ...
        if (input.isEmpty()) {
            ...
            getAESCipher().let { cipher ->
                generateRandomBytes(cipher.blockSize.bytes).let { iv ->
                    cipher.init(Cipher.ENCRYPT_MODE, secretKey, IvParameterSpec(iv))
                    IVCipherText(cipher.iv, cipher.doFinal(input))
                }
            }
        }
...
//        private const val MAX_BIO_KEY_AGE_DAYS = 31
        ...
        fun createNewKey(password: Password): Pair<Salt, IVCipherText> =
            Salt(generateRandomBytes(SALT_SIZE)).let { salt ->
                generatePBKDF2AESKey(
                    salt,
                    KEY_ITERATION_COUNT,
                    password,
