...
fun Password.encrypt(encrypter: (ByteArray) -> IVCipherText = KeyStoreHelperFactory.encrypterProvider) =
    encrypter(String(this.utf8password).toByteArray())

fun String.encrypt(encrypter: (ByteArray) -> IVCipherText = KeyStoreHelperFactory.encrypterProvider) =
    encrypter(this.trim().toByteArray())

fun ByteArray.encrypt(encrypter: (ByteArray) -> IVCipherText = KeyStoreHelperFactory.encrypterProvider) =
    encrypter(this)
...
fun Bitmap.encrypt(encrypter: (ByteArray) -> IVCipherText = KeyStoreHelperFactory.encrypterProvider) =
