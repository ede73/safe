...
start_emulator() {
  ...
  "${ANDROID_HOME}/emulator/emulator" -avd Pixel_2_API_33.avd -no-audio -port "$(get_emu_port)" >/dev/null 2>&1
}

get_emu_serial() {
  sleep 1
  run_adb_command adb devices | grep emulator | cut -f1
}
...
wait_emu_online() {
  ...
  while [ -z "$boot_completed" ]; do
      boot_completed=$(run_adb_command -s "$emulator_serial" shell getprop sys.boot_completed 2>&1 | tr -d '\r')
...
kill_emu() {
  emulator_serial="$1"
  emulatorpid="$2"
  ANDROID_EMULATOR_WAIT_TIME_BEFORE_KILL=5 run_adb_command -s "$emulator_serial" emu kill
  kill $emulatorpid
  wait $emulatorpid
}
